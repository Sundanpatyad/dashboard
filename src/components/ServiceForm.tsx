...
import { X, Plus, Trash2, Upload, Image as ImageIcon, DollarSign, Tag, Type, FileText, Sparkles } from 'lucide-react';
...
const ServiceForm = ({ service, onClose, onSave }: ServiceFormProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const categoryRes = await getData('/api/services/category');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call the parent's save handler which will handle Redux actions
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-gray-700 transform transition-all duration-300 scale-100">
        {/* Header with gradient */}
        <div className="relative bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-700 dark:to-purple-700 p-6 rounded-t-2xl">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-100 dark:text-blue-200 text-sm">
                  {service ? 'Update service details and pricing' : 'Add a new service to your catalog'}
                </p>
              </div>
            </div>
            <button
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 bg-gray-50 dark:bg-gray-900/50">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
              <span className="font-medium">💡 Tip:</span> {service?.image ? 'Upload a new image to replace the current one' : 'Recommended: 800x600px, max 5MB, JPG/PNG format'}
            </p>
          </div>
